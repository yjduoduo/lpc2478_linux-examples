 ...
 *
 */
//#define CONFIG_SERIAL_DCC_CONSOLE 1
...
struct uart_port dcc_ports[];
static void
dcc_poll(void *data)
{
	struct uart_port *port = &dcc_ports[0];
...
static int dcc_startup(struct uart_port *port)
{
	...
	INIT_WORK(&dcc_poll_task, dcc_poll);
	schedule_delayed_work(&dcc_poll_task, 1);
#endif
...
struct uart_port dcc_ports[UART_NR] = {
	{
